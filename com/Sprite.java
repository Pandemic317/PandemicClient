...
public final class Sprite extends DrawingArea
{
    ...
    /*     */   {
    /* 177 */     posX += this.xDrawOffset;
    /* 178 */     posY += this.yDrawOffset;
    /* 179 */     int containerPixel = posX + posY * DrawingArea.width;
    /*     */     
    /* 181 */     int pixelOffset = 0;
    /* 182 */     int height = this.myHeight;
    /* 183 */     int width = this.myWidth;
    /* 184 */     int containerScanSize = DrawingArea.width - width;
    /* 185 */     int spriteScanSize = 0;
    /* 186 */     if (posY < DrawingArea.topY) {
    /* 187 */       int heightLeft = DrawingArea.topY - posY;
    /* 188 */       height -= heightLeft;
    /* 189 */       posY = DrawingArea.topY;
    /* 190 */       pixelOffset += heightLeft * width;
    /* 191 */       containerPixel += heightLeft * DrawingArea.width;
    /*     */     }
    /* 193 */     if (posY + height > DrawingArea.bottomY) {
    /* 194 */       height -= posY + height - DrawingArea.bottomY;
    /*     */     }
    /* 196 */     if (posX < DrawingArea.topX) {
    /* 197 */       int widthLeft = DrawingArea.topX - posX;
    /* 198 */       width -= widthLeft;
    /* 199 */       posX = DrawingArea.topX;
    /* 200 */       pixelOffset += widthLeft;
    /* 201 */       containerPixel += widthLeft;
    /* 202 */       spriteScanSize += widthLeft;
    /* 203 */       containerScanSize += widthLeft;
    /*     */     }
    /*     */     
    /* 206 */     if (posX + width > DrawingArea.bottomX) {
    /* 207 */       int widthLeft = posX + width - DrawingArea.bottomX;
    /* 208 */       width -= widthLeft;
    /* 209 */       spriteScanSize += widthLeft;
    /* 210 */       containerScanSize += widthLeft;
    /*     */     }
    /*     */     
    /* 213 */     if ((width > 0) && (height > 0))
    /* 214 */       drawToContainer(width, height, pixelOffset, this.myPixels, spriteScanSize, containerPixel, DrawingArea.pixels, containerScanSize);
    /*     */   }
       private void drawToContainer(int width, int height, int spritePixel, int[] spriteData, int spriteScanSize, int containerPixel, int[] containerData, int containerScanSize) { for (int y = 0; y < height; y++) {
    	/* 239 */       for (int x = 0; x < width; x++) {
    	/* 240 */         int argb = spriteData[(spritePixel++)];
    	/* 241 */         if (argb != 0) {
    	/* 242 */           int alpha = argb >> 24 & 0xFF;
    	/* 243 */           int transparency = 256 - alpha;
    	/* 244 */           int originalColor = containerData[containerPixel];
    	/* 245 */           containerData[(containerPixel++)] = (((argb & 0xFF00FF) * alpha + (originalColor & 0xFF00FF) * transparency & 0xFF00FF00) + ((argb & 0xFF00) * alpha + (originalColor & 0xFF00) * transparency & 0xFF0000) >> 8);
    	/*     */         } else {
    	/* 247 */           containerPixel++;
    	/*     */         }
    	/*     */       }
    	/* 250 */       containerPixel += containerScanSize;
    	/* 251 */       spritePixel += spriteScanSize;
    	/*     */     }
    	/*     */   }
